...
<div className="relative h-96 max-w-6xl mx-auto bg-gradient-to-br from-blue-50 to-orange-50 rounded-xl shadow-xl overflow-hidden p-6 mb-10">
      {/* Decorative elements */}
      <div className="absolute -top-16 -right-16 w-64 h-64 rounded-full opacity-20" style={{ backgroundColor: secondaryColor }}></div>
      <div className="absolute -bottom-20 -left-20 w-80 h-80 rounded-full opacity-20" style={{ backgroundColor: primaryColor }}></div>
      
      {/* Title */}
      <h2 className="text-2xl sm:text-3xl font-bold text-center relative z-10 mb-8" style={{ color: secondaryColor }}>
      ...
      <div className="relative z-10 flex justify-center">
        <div className="w-full max-w-4xl">
          ...
          {/* Moving timeline with floating elements */}
          <div className="relative mb-20"> {/* Increased margin bottom to prevent overlap */}
            {/* Timeline bar */}
            <div className="h-3 bg-white bg-opacity-60 rounded-full w-full">
              <div 
            ...
            <div className="flex justify-between mt-2">
              {['Plan', 'Design', 'Develop', 'Test', 'Launch'].map((phase, index) => (
                <div 
                  key={index} 
                  className="flex flex-col items-center"
                  style={{
                    transition: 'all 0.3s ease',
                    transform: progress >= index * 25 ? 'translateY(0)' : 'translateY(10px)',
                ...
                >
                  ...
                  <span className="text-xs font-medium mt-1" style={{ color: secondaryColor }}>{phase}</span>
                </div>
              ))}
            </div>
            
            {/* Floating task cards - repositioned to avoid overlap */}
            {[
              { 
                left: '5%', top: '4rem', delay: '0s', 
                color: `bg-white border-${primaryColor}`,
                title: 'Task Planning', 
          ...
          {/* Features row - moved down to avoid overlap */}
          <div className="ml-50 grid grid-cols-3 gap-8 mt-25">
            {[
              ...
              <div 
                key={index} 
                className="bg-white bg-opacity-80 backdrop-blur-sm rounded-lg p-3 text-center shadow-md"
                style={{
                  animation: `slideUp 0.5s ease-out ${index * 0.2}s`,
                  animationFillMode: 'both'
                }}
              >
                <div className="text-xl sm:text-2xl font-bold" style={{ color: primaryColor }}>{stat.percent}</div>
      ...
      <div className="relative z-10 flex justify-center">
        <div className="w-full max-w-4xl">
          ...
          <div className="relative mb-12 sm:mb-20">
            ...
            <div className="sm:hidden mt-4 space-y-2 mb-6">
              {[
                { 
                  title: 'Task Planning', 
           ...
           {/* Features row - single column on iPhone */}
           <div className="sm-hidden grid grid-cols-1 sm:grid-cols-3 gap-2 sm:gap-8">
            {[
              { percent: '85%', label: 'Productivity Increase' },
              { percent: '60%', label: 'Time Saved' },
              { percent: '95%', label: 'Customer Satisfaction' }
              ...
              <div 
                key={index} 
                className="bg-white bg-opacity-80 backdrop-blur-sm rounded-lg p-2 sm:p-3 text-center shadow-md"
                style={{
                  animation: `slideUp 0.5s ease-out ${index * 0.2}s`,
                  animationFillMode: 'both'
                }}
...
<div className="relative h-auto min-h-screen sm:min-h-96 w-full max-w-6xl mx-auto bg-gradient-to-br from-blue-50 to-orange-50 rounded-lg sm:rounded-xl shadow-lg sm:shadow-xl overflow-hidden p-2 sm:p-4 md:p-6 mb-4 sm:mb-10">
      {/* Decorative elements - responsive sizes */}
      <div className="absolute -top-8 sm:-top-16 -right-8 sm:-right-16 w-24 sm:w-48 md:w-64 h-24 sm:h-48 md:h-64 rounded-full opacity-20" style={{ backgroundColor: secondaryColor }}></div>
      <div className="absolute -bottom-8 sm:-bottom-16 -left-8 sm:-left-16 w-32 sm:w-56 md:w-80 h-32 sm:h-56 md:h-80 rounded-full opacity-20" style={{ backgroundColor: primaryColor }}></div>
      
      {/* Title - responsive font size */}
      <h2 className="text-xl sm:text-2xl md:text-3xl font-bold text-center relative z-10 mb-3 sm:mb-6 md:mb-8" style={{ color: secondaryColor }}>
        Best Project Management Software
      </h2>
      
      {/* Main content container */}
      <div className="relative z-10 flex justify-center px-1 sm:px-4">
        <div className="w-full max-w-4xl">
          
          {/* Moving timeline - responsive sizing */}
          <div className="relative mb-8 sm:mb-12 md:mb-20">
            {/* Timeline bar */}
            <div className="h-1.5 sm:h-2 md:h-3 bg-white bg-opacity-60 rounded-full w-full">
              <div 
                className="h-full rounded-full transition-all duration-300"
                style={{ width: `${progress}%`, backgroundColor: primaryColor }}
              ></div>
            ...
            {/* Timeline milestones - responsive spacing and sizing */}
            <div className="flex justify-between mt-1 sm:mt-2">
              {['Plan', 'Design', 'Dev', 'Test', 'Launch'].map((phase, index) => (
                <div 
                  key={index} 
                  ...
                  style={{
                    transition: 'all 0.3s ease',
                    transform: progress >= index * 25 ? 'translateY(0)' : 'translateY(3px)',
                    opacity: progress >= index * 25 ? 1 : 0.5
                  }}
                >
                  <div className="w-4 h-4 sm:w-6 sm:h-6 md:w-8 md:h-8 rounded-full flex items-center justify-center text-white"
                    style={{ 
                      backgroundColor: progress >= index * 25 ? primaryColor : '#CBD5E0'
                    }}
                  >
                    {progress >= index * 25 ? 
                      <CheckCircle size={windowWidth < 640 ? 8 : 16} /> :
                      <span className={`font-bold ${windowWidth < 640 ? 'text-xs' : 'text-sm'}`}>{index + 1}</span>
                    }
                  </div>
                  <span className="text-xs sm:text-sm font-medium mt-1" style={{ color: secondaryColor }}>
                    {windowWidth < 350 && index === 2 ? 'Dev' : phase}
                  </span>
                </div>
              ))}
            </div>
            
            {/* Task cards - desktop version for larger screens */}
            ...
            {/* Task cards - tablet and mobile version */}
            <div className="md:hidden mt-4 space-y-2 mb-4 sm:mb-6 w-full">
              {[
                { 
                  title: windowWidth < 350 ? 'Tasks' : 'Task Planning', 
                  icon: <Calendar size={windowWidth < 350 ? 10 : 12} style={{ color: primaryColor }} />,
                  text: "Plan your project roadmap"
                },
                { 
                  title: windowWidth < 350 ? 'Time' : 'Time Tracking', 
                  icon: <Clock size={windowWidth < 350 ? 10 : 12} style={{ color: primaryColor }} />,
                  text: "Track hours & deadlines"
                },
                { 
                  title: windowWidth < 350 ? 'Team' : 'Team Collab', 
                  icon: <Users size={windowWidth < 350 ? 10 : 12} style={{ color: primaryColor }} />,
                  text: "Work together seamlessly"
                },
                { 
                  title: windowWidth < 350 ? 'Stats' : 'Analytics', 
                  icon: <BarChart2 size={windowWidth < 350 ? 10 : 12} style={{ color: primaryColor }} />,
                  text: "Track project metrics"
                }
              ].map((card, index) => (
                <div 
                  key={index}
                  className="bg-white p-1.5 sm:p-2 rounded-lg shadow-md border flex items-center"
                  style={{
                    borderColor: primaryColor,
                    borderWidth: '1px',
                    animation: `slideUp 0.5s ease-out ${index * 0.15}s`,
                ...
                >
                  <div className="mr-1.5 sm:mr-2 p-1 rounded flex-shrink-0">
                    {card.icon}
                  </div>
                  <div className="min-w-0 flex-1">
                    <div className="text-xs font-bold truncate" style={{ color: secondaryColor }}>{card.title}</div>
                    <p className="text-xs truncate" style={{ color: secondaryColor }}>
                      {windowWidth < 350 ? card.text.split(' ').slice(0, 2).join(' ') : card.text}
                    </p>
                  </div>
                </div>
              ))}
          ...
          <div className="flex justify-center w-full px-1 sm:px-0">
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 sm:gap-4 md:gap-8 w-full max-w-md sm:max-w-2xl mx-auto">
              {[
                { percent: '85%', label: 'Productivity Increase' },
                { percent: '60%', label: 'Time Saved' },
                { percent: '95%', label: 'Customer Satisfaction' }
                ...
                >
                  <div className="text-lg sm:text-xl md:text-2xl font-bold" style={{ color: primaryColor }}>{stat.percent}</div>
                  <div className="text-xs sm:text-sm" style={{ color: secondaryColor }}>
                    {windowWidth < 350 ? 
                      (index === 0 ? 'Productivity' : index === 1 ? 'Time Saved' : 'Satisfaction') : 
                      stat.label}
                  </div>
                </div>
              ))}
